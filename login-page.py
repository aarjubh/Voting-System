...
'''def login():
    ...
    try:
        conn = sqlite3.connect(database_file)
        cursor = conn.cursor()
        cursor.execute("SELECT otp_secret, generated_otp FROM users WHERE voter_id=?", (voter_id,))
        user = cursor.fetchone()
        
        if user:
            #stored_otp_secret = user[0]
            stored_generated_otp = user[0]
            
            if entered_otp == stored_generated_otp:#and entered_otp == generate_otp(stored_otp_secret):
                print("Login successful. Redirecting to the main page.")
